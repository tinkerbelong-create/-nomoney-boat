...
import type { CreatureRow, TankRow } from './aquarium';
...
// 水槽
//
// 型と定数は lib/aquarium.ts にある（クライアントからも読むため）。
// DBを叩くのはこっち。
// ---------------------------------------------------------------------

/** 図鑑。未取得も含めて324体すべて返る */
export async function getCreatureBook(userId: string): Promise<CreatureRow[]> {
  const supabase = await supabaseServer();
  const { data, error } = await supabase.rpc('creature_book', { p_user_id: userId });
  if (error) throw error;
  return (data ?? []) as CreatureRow[];
}

/** いま水槽に入っているものだけ */
export async function getTank(userId: string): Promise<TankRow[]> {
  const supabase = await supabaseServer();
  const { data, error } = await supabase.rpc('tank_view', { p_user_id: userId });
  if (error) throw error;
